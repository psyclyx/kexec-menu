...
mod tests {
    ...
    fn fallback_uses_first_entry() {
        let trees = vec![source(
            "disk",
            vec![leaf("/mnt/boot/gen1", &["default", "gaming"], 100)],
        )];
        let sel = resolve_default(&trees, None).unwrap();
        assert_eq!(sel.entry_name, "default");
    }
    ...
    fn efi_var_parent_gone_falls_to_global() {
        let trees = vec![source(
            "disk",
            vec![leaf("/mnt/boot/other/gen1", &["default"], 100)],
        )];
        let var = BootSelection {
    ...
    fn efi_var_with_nested_tree() {
        let trees = vec![source(
            ...
            vec![
                ...
                TreeNode::Dir {
                    name: "arch".into(),
                    children: vec![leaf("/mnt/boot/arch/gen1", &["default"], 300)],
                },
            ],
        )];
        let var = BootSelection {
    ...
    fn efi_var_single_sibling_selects_itself() {
        let trees = vec![source(
            "disk",
            vec![leaf("/mnt/boot/nixos/gen1", &["default", "gaming"], 100)],
        )];
        let var = BootSelection {
            leaf_path: PathBuf::from("/mnt/boot/nixos/gen1"),
            entry_name: "gaming".into(),
    ...
    fn single_entry_default() {
        ...
        let trees = vec![source(
            "only-disk",
            vec![leaf("/mnt/boot/gen1", &["the-only-one"], 42)],
        )];
        let sel = resolve_default(&trees, None).unwrap();
    ...
    fn multi_source_default_nested() {
        ...
        let trees = vec![
            ...
            source("disk-c", vec![leaf("/mnt3/boot/gen1", &["flat"], 50)]),
        ];
        let sel = resolve_default(&trees, None).unwrap();
